...
export default function TableListDadosColetados({
  handleModal,
  Header,
}: IProps) {
  ...
  const [historyDataColetado, setHistoryDataColetado] =
    useState<CommunityData[]>();
  async function populateComunidadeHistory() {
    try {
      const resp_get_comunidade = await coletaAPI.get<CommunityData[]>(
        `/textos-coletados/${comunidade?.data.id}`,
      );
        ...
        <TouchableNativeFeedback>
          ...
          >
            <VStack>
              ...
              >
                Data de coleta:{" "}
                {format(new Date(item.data_coletado), "dd/MM/yyyy")}
              </Text>
            </VStack>
